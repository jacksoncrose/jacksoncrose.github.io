---
# 2026-08-20 de-AI pass, flag 5, his call: "carry real topographic
# legibility" -> "keep the terrain readable" (hollow intensifier plus
# naming-not-describing). The rest of the sentence is untouched.
# 2026-08-05 de-AI pass: Foglands blurbs reworded (KEEP IN SYNC with the
# two gallery files — same strings, four sites total) and the
# pasture-naming sentence rewritten; all his picks.
# Body condensed from Jackson's write-up, 2026-07-27, supplied in his own
# words for the site. Publish and client naming cleared by Jackson the same
# day. Full write-up facts recorded in AGENTS.md (ROONEY block).
# ArcGIS Pro in tools is inferred from the sheets — confirm with Jackson.
# 2026-08-14: Problem middle sentence rewritten at Jackson's direction (the
# "The spatial knowledge that runs the operation, meaning..." construction
# read like AI; he rejected "scattered files" and picked the heads-only
# wording). The sentence after it is unchanged.
title: "Rooney Angus Ranch Atlas"
summary: "Ranch atlas and poster cartography for Rooney Angus: four division maps on a custom 10 m hillshade with one symbol system across a threefold scale range, a wall-scale poster, and an interactive Google Earth environment transferred to the ranch's ownership."
year: 2026
tier: 3
role: "Project lead and cartographer"
tools: ["ArcGIS Pro", "Google Earth"]
client: "Rooney Angus"
clientPermission: "granted"
hasPage: true
cover:
  src: "../../assets/images/rooney-north-hillshade.png"
  alt: "Division map of Rooney North on a grayscale hillshade: a cased orange boundary enclosing white owned parcels and hatched public lease, crimson ranch roads threading between labeled pastures, with aerial-imagery inset panels for the headquarters and subdivided fields."
# Every value restated from the body copy below; see the stats schema note.
stats:
  - value: "4"
    label: "division maps, one symbol system"
  - value: "1:17k–1:53k"
    label: "a threefold spread in scale"
  - value: "10 m"
    label: "DEM behind the custom hillshade"
draft: false
---

import CompareFigure from '../../components/CompareFigure.astro';
import SheetFigure from '../../components/SheetFigure.astro';
import foglandsHillshade from '../../assets/images/rooney-foglands-hillshade.png';
import foglandsImagery from '../../assets/images/rooney-foglands-imagery.jpg';
import poster from '../../assets/images/rooney-ranches-poster.jpg';

## Problem

Rooney Angus operates four ranch divisions across a wide area of central
Montana, on a mix of owned ground, private lease, and public lease. Pasture
names, fence lines, gate locations, water infrastructure, irrigation
history, and the two-track roads lived mostly in people's heads. There was
no single reference the crew, family, and managers could all work from.

## Approach

{/* 2026-08-15 second de-AI pass, his "implement all recommendations":
    the fragment opener "A purpose-built basemap." cut, and the next
    paragraph's announce-then-prove opener "One symbol system holds across
    four very different maps." cut, with "A single visual grammar" becoming
    "One symbol system" so the point and plain term survive in one
    sentence. Scales and counts untouched. */}

I produced a custom hillshade from the USGS 1/3
arc-second (10 m) DEM and tuned it as a desaturated grayscale surface.
Terrain is what orients someone on a ranch, so the basemap had to keep
the terrain readable while staying quiet enough that fences, roads,
and boundaries read cleanly on top of it. Everything was built in
NAD83(2011) UTM Zone 12N so the full series shares one coordinate
framework.

The divisions
range from Democrat at 1:17,000 to Rooney North at 1:53,000, a threefold
spread in scale with large differences in feature density. One symbol
system carries all of them: a cased orange ranch boundary, crimson ranch
roads, dashed fences, gate points, fill patterns distinguishing owned
ground from private and public lease, and a consistent point taxonomy for
water and range infrastructure. A crew member who learns to read one sheet
can read all of them.

{/* Labels, blurbs, and alt text reuse the cleared gallery entries for this
    pair verbatim — no new claims. The pair registers: both 2400x1553. */}

<CompareFigure
	title="Foglands"
	meta="One division on its two basemaps · 1:29,000"
	a={{
		image: foglandsHillshade,
		alt: 'Division map of Foglands on a grayscale hillshade: white and hatched parcels inside a cased orange boundary, crimson ranch roads following incised drainages, and pasture names such as Deer Gulch and Bachelor Gulch set along the ridges they describe.',
		label: 'Hillshade',
		blurb: 'Foglands at 1:29,000. The pastures are named for the terrain: Deer Gulch, Rocky Hollow, Bachelor Gulch, Horse Creek. Hatched private lease sits inside the ranch boundary.',
	}}
	b={{
		image: foglandsImagery,
		alt: 'Division map of Foglands over aerial imagery: dark conifer stands filling the gulches against open tan rangeland and cultivated fields, with the cased orange ranch boundary, white ranch roads, and the same pasture labels as the hillshade version.',
		label: 'Imagery',
		blurb: 'The same division over aerial imagery, where timber, meadow, and cultivated ground separate out in a way the hillshade cannot show.',
	}}
/>

Where scale broke down, aerial imagery inset panels with leader lines give
the headquarters complexes and the tightly subdivided fields on Rooney
North the detail they need without compressing the main map. The maps also
label several hundred pasture and field names in the ranch's own
vocabulary, from Bachelor Gulch and Lambing Camp to Hayland Jock's and the
Susie's fields. Those names are how the ranch already talks about its land, and the maps
had to record them exactly to work as a shared reference. The
ranch wordmark and brand color carry through the boundary symbology and
sheet furniture, so the series reads as the ranch's own document.

## What was delivered

Four division maps (Rooney North, Rooney South, Foglands, Democrat), each
with scale bar, north arrow, and projection and DEM provenance noted; a
large-format poster placing all four divisions in regional context on
satellite imagery, produced in alternate label treatments so the ranch
could choose what read best at wall scale; and an interactive Google Earth
environment built from the same dataset and transferred to the ranch's
ownership, giving the team a way to explore their own data without GIS
software or a license.

{/* Caption wording from the cleared gallery entry for the poster. */}

<SheetFigure
	src={poster}
	alt="Wall-scale poster of the four Rooney Angus divisions outlined in orange over satellite imagery of central Montana, pasture boundaries and ranch roads within each, and a headquarters star on Rooney South."
	caption="The wall-scale poster at 1:29,000: all four divisions in regional context on satellite imagery, produced in alternate label treatments so the ranch could choose what read best."
	linkLabel="Open the full Rooney Angus Ranches poster"
/>

Foundational digitizing and GIS construction were carried out by GCF
staff. I directed that work and produced all final cartographic and
interactive deliverables.

## Outcome

The ranch came out of phase one with a projected, structured spatial
dataset it owns, a printed atlas the crew can use in trucks and shops, a
wall-scale poster for the office, and an interactive environment for
planning. Because the data was built to be extended rather than delivered
as a finished snapshot, phase two options are on the table, including
attributing infrastructure by fence and road type, refining feature
alignment, and on-site collection for pasture-level resolution.
